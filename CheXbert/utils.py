...
def get_weighted_f1_weights(train_path_or_csv):
    ...
    """
    if isinstance(train_path_or_csv, str):  # If `train_path_or_csv' is a string, means it's a path
        df = pd.read_csv(train_path_or_csv)
    else:
        df = train_path_or_csv   # If `train_path_or_csv' is a dataframe, use it directly
    df.replace(0, 2, inplace=True)  # Replace 0 with 2, act on original dataframe not return a new one
    df.replace(-1, 3, inplace=True)
    df.fillna(0, inplace=True)  # Fill NaN values with 0
    # Original form in `train_path_or_csv`: {nan: "Blank", 1: "Positive", 0: "Negative", -1: "Uncertain"}
    # Now: {0: "Blank", 1: "Positive", 2: "Negative", 3: "Uncertain"}
    ...
    for cond in CONDITIONS:
        ...
        # weights[0]: number of negative labels

        mask = col == 3
        weights.append(mask.sum())
        # weights[1]: number of uncertain labels

        mask = col == 1
        weights.append(mask.sum())
        # weights[2]: number of positive labels
